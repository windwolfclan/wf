...
namespace psd
{
    ...
    void Image::load( const std::string& /*filename*/ )
    {
        ...
        //
        //
        // if (loadtga(filename, (void**)&m_data, &m_width, &m_height))
        // {
        //    char name[256];
    ...
    {
        return const_cast<uint32_t*>( m_data.data() ) + y * m_width;
    }
    ...
    {
        int32_t x = (int)floor( u * ( m_width - 1 ) );
        int32_t y = (int)floor( v * ( m_height - 1 ) );
    ...
    {
        int32_t nx = m_width >> 1;
        int32_t ny = m_height >> 1;
    ...
    void Image::scaled( const Image& image ) const
    {
        ...
        int32_t dx = ( w << 16 ) / m_width;
        int32_t dy = ( h << 16 ) / m_height;

        int32_t iy = 0;
        for ( int32_t dstY = 0; dstY < m_height; dstY++ )
        {
            int32_t y = iy >> 16;
            ...
            int32_t ix = 0;
            for ( int32_t dstX = 0; dstX < m_width - 1; dstX++ )
            {
                int32_t x = ix >> 16;
            ...
            dst[ m_width - 1 ] = blend( src1[ w - 1 ], src2[ w - 1 ], sy );
    ...
    {
        int32_t width = std::min<int>( m_width, image.getWidth() );
        int32_t height = std::min<int>( m_height, image.getHeight() );
    ...
    {
        int32_t width = std::min<int>( m_width - posX, image.getWidth() );
        int32_t height = std::min<int>( m_height - posY, image.getHeight() );

        int32_t endx = m_width - posX;
        int32_t endy = m_height - posY;
    ...
    {
        uint32_t* src = m_data.data();
        uint32_t* src0 = m_data.data() + ( m_height - 1 ) * m_width;
        uint32_t* src1 = m_data.data();
        uint32_t* src2 = m_data.data() + m_width;

        uint32_t* dst = new uint32_t[ m_width * m_height ];

        for ( int32_t y = 0; y < m_height; y++ )
        {
            dst[ 0 ] = calcNormal( z, src1[ m_width - 1 ], src1[ 1 ], src0[ 0 ], src2[ 0 ] );
            for ( int32_t x = 1; x < m_width - 1; x++ )
            {
                dst[ x ] = calcNormal( z, src1[ x - 1 ], src1[ x + 1 ], src0[ x ], src2[ x ] );
            }
            dst[ m_width - 1 ] = calcNormal( z, src1[ m_width - 2 ], src1[ 0 ], src0[ m_width - 1 ], src2[ m_width - 1 ] );
    ...
    void Image::buildDeltaMap()
    {
        ...
        {
            dst[ 0 ] =
                ( ( 128 + ( src1[ m_width - 1 ] & 0xff ) * s - ( src1[ 1 ] & 0xff ) * s ) << 16 )
            ...
            dst[ m_width - 1 ] =
                ( ( 128 + ( src1[ m_width - 2 ] & 0xff ) * s - ( src1[ 0 ] & 0xff ) * s ) << 16 )
                | ( ( 128 + ( src0[ m_width - 1 ] & 0xff ) * s - ( src2[ m_width - 1 ] & 0xff ) * s ) << 8 );
