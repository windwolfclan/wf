...
namespace wf
{
	...
	{
		m_vs = L"lightmap_vs.hlsl";
		m_ps = L"lightmap_ps.hlsl";
		m_vs_main = "lightmap_vs";
		m_ps_main = "lightmap_ps";
	}
	...
	bool LightmapShader::Render( ID3D11DeviceContext* _context, int _index_count, XMMATRIX _w, XMMATRIX _v, XMMATRIX _p, ID3D11ShaderResourceView** _srvs )
	{
	...
	bool LightmapShader::SetShaderParameters( ID3D11DeviceContext* _context, XMMATRIX _w, XMMATRIX _v, XMMATRIX _p, ID3D11ShaderResourceView** _srvs )
	{
		...
		_context->PSSetShaderResources( 0, 2, _srvs );

		return true;
	}

	void LightmapShader::RenderShader( ID3D11DeviceContext* _context, int _index_count )
	{
	...
	bool LightmapShader::InitializeLayout( ID3D11Device* _device, ID3D10Blob*& _blob )
	{
	...
	bool LightmapShader::InitializeBuffers( ID3D11Device* _device )
	{
		HRESULT hr{ S_OK };
	...
	void LightmapShader::ReleaseBuffers()
	{
