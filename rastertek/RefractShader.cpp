...
namespace wf
{
	...
	{
		m_vs = L"refract_vs.hlsl";
		m_ps = L"refract_ps.hlsl";
		m_vs_main = "refract_vs";
		m_ps_main = "refract_ps";
	}

	RefractShader::RefractShader( const RefractShader& )
	{
	}
	...
	{
		if ( !SetShaderParameters( _context, _w, _v, _p, _srv, _light_direction, _ambient, _diffuse, _clip_plane ) )
		{
			return false;
		}
	...
	bool RefractShader::SetShaderParameters(
		ID3D11DeviceContext* _context,
		XMMATRIX _w,
		XMMATRIX _v,
		XMMATRIX _p,
	...
	)
	{
		...
		{
			D3D11_MAPPED_SUBRESOURCE mappedResource;
			if ( SUCCEEDED( _context->Map( m_clip_plane_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource ) ) )
			{
				

				_context->Unmap( m_clip_plane_buffer, 0 );
			}

			UINT slot = 1;
			_context->VSSetConstantBuffers( slot, 1, &m_clip_plane_buffer );
		}

		{
			...
			if ( SUCCEEDED( _context->Map( m_light_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource ) ) )
			{
				...
				data->light_direction = _light_direction;
				data->padding = 0.0f;
	...
	void RefractShader::RenderShader( ID3D11DeviceContext* _context, int _index_count )
	{
	...
	bool RefractShader::InitializeLayout( ID3D11Device* _device, ID3D10Blob*& _blob )
	{
	...
	bool RefractShader::InitializeBuffers( ID3D11Device* _device )
	{
		HRESULT hr{ S_OK };
		...
		hr = utility::CreateDynamicWriteBuffer( _device, m_clip_plane_buffer, sizeof( ClipPlaneBufferType ) );
		if ( FAILED( hr ) )
		{
			return false;
		}
	...
	void RefractShader::ReleaseBuffers()
	{
		SAFE_RELEASE( m_sampler_state );
		SAFE_RELEASE( m_clip_plane_buffer );
