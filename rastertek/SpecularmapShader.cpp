...
namespace wf
{
	...
	{
		m_vs = L"specular_vs.hlsl";
		m_ps = L"specular_ps.hlsl";
		m_vs_main = "specular_vs";
		m_ps_main = "specular_ps";
	}

	SpecularShader::SpecularShader( const SpecularShader& )
	{
	}
	...
	bool SpecularShader::SetShaderParameters( ID3D11DeviceContext* _context, XMMATRIX _w, XMMATRIX _v, XMMATRIX _p, ID3D11ShaderResourceView** _srv, XMFLOAT3 _camera_position, XMFLOAT4 _diffuse, XMFLOAT4 _specular, float _specular_power, XMFLOAT3 _light_dir )
	{
...

		{
			...
			if ( SUCCEEDED( _context->Map( m_camera_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource ) ) )
			{
				...
				data->position = _camera_position;
				data->padding = 0.0f;
		...
		_context->PSSetShaderResources( 0, 3, _srv );

		return true;
	}

	void SpecularShader::RenderShader( ID3D11DeviceContext* _context, int _index_count )
	{
	...
	bool SpecularShader::InitializeLayout( ID3D11Device* _device, ID3D10Blob*& _blob )
	{
	...
	bool SpecularShader::InitializeBuffers( ID3D11Device* _device )
	{
		HRESULT hr{ S_OK };
	...
	void SpecularShader::ReleaseBuffers()
	{
